...
import type { AnalyzeResponse } from "./api_client.js";
...
export class ATSChecker {
  ...
  private jdTextarea: HTMLTextAreaElement;
  private analyzeButton: HTMLButtonElement;
  private onAnalyze: (jdText: string) => Promise<void>;
  ...
  constructor(
    ...
    this.jdTextarea = document.getElementById(
      jdTextareaId,
    ) as HTMLTextAreaElement;
    this.analyzeButton = document.getElementById(
      analyzeButtonId,
  ...
  display(result: AnalyzeResponse): void {
  ...
  private _bindEvents(): void {
    ...
    this.jdTextarea.addEventListener("input", () => {
      ...
      this.debounceTimer = setTimeout(async () => {
        const jd = this.jdTextarea.value.trim();
        if (jd.length > 100) await this.onAnalyze(jd);
      }, 1200);
    });
  }
  ...
  private _scoreColor(score: number): string {
    ...
    if (score >= 55) return "#1a5fa8";
    if (score >= 35) return "#633806";
    return "#791F1F";
  }
}
